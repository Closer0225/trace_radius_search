...
std::string savePointCloud(const std::string& fileName, std::vector<query_t>&pPoints, std::vector<float3>&rsnormals, int query_num) {
    //需要从utf8转成ascii
    ...
    if (fp)
    {
        ...
        {
            fprintf(fp, "%f %f %f %f %f %f\n", pPoints[i].position.x, pPoints[i].position.y,
                pPoints[i].position.z, rsnormals[i].x, rsnormals[i].y, rsnormals[i].z);
        }
        fclose(fp);
    }
    else
    ...
    float_t pRadii, int32_t knn = -1)
{
    ...
    if (knn == -1)
    {
        ...
        savePointCloud("../data/normal.asc", queries, outNormals, queries.size());
    }
    else
    {
        ms_search_timer = index.truncated_knn(queries, knn, indices, dists);
...
/*
lucy   no sort   928   928   929
lucy      sort   802   819   815
*/