...
namespace bvh_radSearch
{
        ...
        float_t* pGaS_size)
    {
        ...
        this->mNormals.alloc(pSamples.size());
        this->mParams.samplePos = this->mSamples.getPtr();
        this->mParams.normals = this->mNormals.getPtr();
    ...
    float_t bvh_index::radius_search_normals(
        std::vector<query_t>& pQueries,
        std::vector<float3>& pNormals)
    {
        this->mSBT.raygenRecord = this->mShaders[MODULE_E::RAD_SEARCH_NORMAL].gen_rec.getRawPtr();
        this->mSBT.missRecordBase = this->mShaders[MODULE_E::RAD_SEARCH_NORMAL].miss_rec.getRawPtr();
        this->mSBT.hitgroupRecordBase = this->mShaders[MODULE_E::RAD_SEARCH_NORMAL].hit_rec.getRawPtr();

        cudaBuffer<query_t> queries;
        queries.alloc(pQueries.size());
        queries.copyHostToDevice(pQueries.data());
        sortmorton(queries, pQueries.size());
        queries.copyDeviceToHost(pQueries.data());
        float_t maxf = std::numeric_limits<float_t>::max();
        ...
        CUDA_SYNC_CHECK();
        float_t elapsed_ms = timer.stop();
        pNormals = mNormals.getData();
        return elapsed_ms;
    }

    float_t bvh_index::radius_search_smooth(
        std::vector<query_t>& pQueries,
        std::vector<float3>& pNormals)
    {
        this->mSBT.raygenRecord = this->mShaders[MODULE_E::RAD_SEARCH_SMOOTH].gen_rec.getRawPtr();
        this->mSBT.missRecordBase = this->mShaders[MODULE_E::RAD_SEARCH_SMOOTH].miss_rec.getRawPtr();
        this->mSBT.hitgroupRecordBase = this->mShaders[MODULE_E::RAD_SEARCH_SMOOTH].hit_rec.getRawPtr();
    ...
    float_t bvh_index::knn_search_normals(std::vector<query_t>& pQueries,
        uint32_t knn,
        std::vector<float3>& pNormals)
    {
        this->mSBT.raygenRecord = this->mShaders[MODULE_E::KNN_SEARCH_NORMAL].gen_rec.getRawPtr();
        this->mSBT.missRecordBase = this->mShaders[MODULE_E::KNN_SEARCH_NORMAL].miss_rec.getRawPtr();
        this->mSBT.hitgroupRecordBase = this->mShaders[MODULE_E::KNN_SEARCH_NORMAL].hit_rec.getRawPtr();
    ...
    float_t bvh_index::knn_search_smooth(std::vector<query_t>& pQueries,
        uint32_t knn,
        std::vector<float3>& pNormals)
    {
        this->mSBT.raygenRecord = this->mShaders[MODULE_E::KNN_SEARCH_SMOOTH].gen_rec.getRawPtr();
        this->mSBT.missRecordBase = this->mShaders[MODULE_E::KNN_SEARCH_SMOOTH].miss_rec.getRawPtr();
        this->mSBT.hitgroupRecordBase = this->mShaders[MODULE_E::KNN_SEARCH_SMOOTH].hit_rec.getRawPtr();
